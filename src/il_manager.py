...
def manage_il(
    ...
    **kwargs
    ...
    # Step 1: Tally IL slots and categorise who's on them
    healthy_on_il = []    # currently on IL but no longer injured → activate
    still_injured_on_il = []  # on IL and still injured → keep, counts toward used
    players_needing_il = []   # injured but NOT yet on IL → move to IL
    il_slots_used = 0
    ...
    for player in roster:
        ...
        if selected_pos in IL_SLOT_NAMES:
            ...
            if status not in IL_ELIGIBLE_STATUSES and status != "DTD":
                healthy_on_il.append(player)   # Recovered — should activate
            else:
                still_injured_on_il.append(player)  # Still injured — keep on IL
        elif status in IL_ELIGIBLE_STATUSES:
            players_needing_il.append(player)

    il_slots_total = kwargs.get('il_slots_total', 3)
    il_slots_available = il_slots_total - il_slots_used

    # Step 2: Activate healthy players from IL (frees their slots)
    for player in healthy_on_il:
        status = player.get("status", "")
        move = {
            "player_id": player["player_id"],
            "player": player["name"],
            "player_name": player["name"],  # alias for email compatibility
            "action": "activate",
